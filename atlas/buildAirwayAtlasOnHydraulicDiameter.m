% 
% build pediatric airway atlas on Hydraulic Diameter curves
% Author: Yi Hong
...
results_prefix = '/playpen/Project/airwayAtlas/Pediatric_Airway_Atlas_Code/results_perimeter/Atlas36_68_19_HD'; % output location
gaussian_sigma = 36;    % band-width for gaussian function, corresponding to sigma
...
for icase = 1:length(cases)
    ...
    tmpStr = sprintf( '%s/Contour%s/%s_Perimeter.txt', data_prefix, cases{icase}, cases{icase} );
    perimeterFile{icase} = tmpStr;
...
params.perimeterFile = perimeterFile;    % perimeter of cross section's contour
...
buildAirwayAtlasForEachSubjectHD( params );
